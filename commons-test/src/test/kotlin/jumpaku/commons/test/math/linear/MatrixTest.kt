...
class MatrixTest {
    ...
    fun buildSparse(data: List<List<Double>>): Map<Matrix.Sparse.Key, Double> {
        ...
        for (i in 0 until data.size) for (j in 0 until data[0].size) data[i][j].let { if (it != 0.0) c[Matrix.Sparse.Key(
            i,
            j
        )] = it }
        return c
